...
nrgMenu * nrgMenu::create(std::vector<std::string> listInput, std::string fontFile, int limitInput, int fontSizeInput, int spacingInput, cocos2d::Color4B textColor1Input, cocos2d::Color4B textColor2Input, bool highlightFirstOption)
{
...
nrgMenu * nrgMenu::create(std::string listInput, std::string fontFile, int limitInput, int fontSizeInput, int spacingInput, cocos2d::Color4B textColor1Input, cocos2d::Color4B textColor2Input, bool highlightFirstOption)
{
...
nrgMenu::nrgMenu(std::vector<std::string> listInput, std::string fontFile, int limitInput, int fontSizeInput, int spacingInput, cocos2d::Color4B textColor1Input, cocos2d::Color4B textColor2Input, bool highlightFirstOption)
{
	font = fontFile;
	...
	if(highlightFirstOption)
		option.at(0)->setTextColor(textColor2Input);
	else
		option.at(0)->setTextColor(textColor1Input);

	addChild(option.at(0));
	option.at(0)->setAnchorPoint(cocos2d::Vec2(0, 0));
	option.at(0)->setPosition(0, 0);
	...
	for (int iii = 1; iii < numberOfOptions; iii++)
	{
		...
		option.at(iii)->setTextColor(textColor1Input);
		addChild(option.at(iii));
		option.at(iii)->setAnchorPoint(cocos2d::Vec2(0, 0));
		option.at(iii)->setPosition(0, (-iii * scale * letterHeight) - iii * spacing);
	}

	float top = option.at(0)->getPositionY();
	float bottom = option.at(numberOfOptions - 1)->getPositionY() - letterHeight;
	...
	{
		width1 = option.at(currentLine)->getContentSize().width; 
		width2 = option.at(longestLine)->getContentSize().width;
...
void nrgMenu::ChangeMenu(std::vector<std::string> listInput, int limitInput, bool highlightFirstOption)
{
	...
	option.push_back(cocos2d::Label::createWithTTF(list[0], font, fontSize, cocos2d::Size::ZERO, cocos2d::TextHAlignment::LEFT, cocos2d::TextVAlignment::TOP));
	if (highlightFirstOption)
		option.at(0)->setTextColor(textColor2);
	else
		option.at(0)->setTextColor(textColor1);
	...
	{
		option.push_back(cocos2d::Label::createWithTTF(list[iii], font, fontSize, cocos2d::Size::ZERO, cocos2d::TextHAlignment::LEFT, cocos2d::TextVAlignment::TOP));
		option.at(iii)->setTextColor(textColor1);
		option.at(iii)->setBlendFunc(cocos2d::BlendFunc::ALPHA_NON_PREMULTIPLIED);
		addChild(option.at(iii));
		option.at(iii)->setAnchorPoint(cocos2d::Vec2(0, 0));
		option.at(iii)->setPosition(0, (-iii * scale * letterHeight) - iii * spacing);
	}
...
void nrgMenu::addEvents()
{
	...
	touchListener->onTouchBegan = [&](cocos2d::Touch* touch, cocos2d::Event* event)
	{
		...
		for (int iii = 0; iii < numberOfOptions; iii++)
		{
			...
			{
				option[iii]->setTextColor(textColor2);
				selection = iii;
				itemPressed = true;
			}
			else
				option.at(iii)->setTextColor(textColor1); //If not selected, set text to default color
		}
	...
	{	
		...
		option.at(selection)->setTextColor(textColor1);
	};
...
{
	option[value]->setTextColor(textColor2);
	selection = value;
	return;
}
...
{
	option[selection]->setTextColor(textColor2);
	return;
}
...
void nrgMenu::ChangeTextColor1(cocos2d::Color4B color, bool highlightOption)
{
	textColor1 = color;
	for (int iii = 0; iii < option.size(); iii++)
		option[iii]->setTextColor(color);
...
void nrgMenu::ChangeTextColor2(cocos2d::Color4B color, bool highlightOption)
{
	textColor2 = color;
