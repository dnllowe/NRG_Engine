...
{
	createEngine();
	AndroidAudioClips::SetAndroidDeviceBufferSize(); //Calls Java SetDeviceBufferSize function, which will call the C++ function again with the correct data
	deviceBufferSize = AndroidAudioClips::GetDeviceBufferSize();

#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	//LOGD("Device Buffer Size Returned...");

	//Check to see if sample rate matches an OpenSL ES sample rate
	switch (deviceBufferSize)
	{
	...
	case SL_SAMPLINGRATE_192:
		...
		break;
	default:
		//LOGD("Program provided device buffer size does not match an OpenSL ES defined setting");
		break;
	}
#endif
	createAudioPlayer();
...
bool OSLESMediaPlayer::createAudioPlayer()
{
	...
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject1, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 1 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject2, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 2 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject3, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 3 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject4, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 4 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject5, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 5 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject6, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 6 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject7, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
	//LOGD("Buffer 7 created");
	(void)result;
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &bufferObject8, &audioSrc, &audioSnk, 1, ids, req);
	assert(SL_RESULT_SUCCESS == result);
...
void OSLESMediaPlayer::Play(std::string clip)
{
	...
	if (clip == "miss1")
	{
		nextBuffer = (short *)miss1;
		nextSize = sizeof(miss1);
	}
	else if (clip == "miss2")
	{
		nextBuffer = (short *)miss2;
		nextSize = sizeof(miss2);
	}
	else if (clip == "hitEnemy1")
	{
		nextBuffer = (short *)hitEnemy1;
		nextSize = sizeof(hitEnemy1);
	}
	else if (clip == "hitEnemy2")
	{
		nextBuffer = (short *)hitEnemy2;
		nextSize = sizeof(hitEnemy2);
	}
	else if (clip == "hitEnemy3")
	{
		nextBuffer = (short *)hitEnemy3;
		nextSize = sizeof(hitEnemy3);
	}
	else if (clip == "tick")
	{
		nextBuffer = (short *)tick;
		nextSize = sizeof(tick);
	}
	else if (clip == "multiplierUp")
	{
		nextBuffer = (short *)multiplierUp;
		nextSize = sizeof(multiplierUp);
	}
	else if (clip == "multiplierDown")
	{
		nextBuffer = (short *)multiplierDown;
		nextSize = sizeof(multiplierDown);
	}
	else if (clip == "chime")
	{
		nextBuffer = (short *)chime;
		nextSize = sizeof(chime);
	}
	else if (clip == "hitPlayer1")
	{
		nextBuffer = (short *)hitPlayer1;
		nextSize = sizeof(hitPlayer1);
	}
	else if (clip == "hitPlayer2")
	{
		nextBuffer = (short *)hitPlayer2;
		nextSize = sizeof(hitPlayer2);
	}
	else if (clip == "hitPlayer3")
	{
		nextBuffer = (short *)hitPlayer3;
		nextSize = sizeof(hitPlayer3);
	}
	else if (clip == "block")
	{
		nextBuffer = (short *)block;
		nextSize = sizeof(block);
	}
